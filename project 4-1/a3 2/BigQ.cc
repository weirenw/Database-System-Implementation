...
Qin(in), Qout(out), Qsortorder(sortorder), Qrunlen(runlen), mysorter(sortorder), mysorter2(sortorder) 
{
	curPage = new Page();
	theFile = new File();
	tmpFileStoreSortedRuns = (char*)tmpfName();
	theFile->Open(0, tmpFileStoreSortedRuns);  // need a random name otherwise two or more bigq's would crash
